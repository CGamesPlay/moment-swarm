...
def _(mo, os):
    ...
    sweep_params = mo.ui.text_area(
        value="EXPLORE_TIMEOUT=300,600,900\nTRAIL_STRENGTH=50,100,150\nSCAN_STEPS=20,40,60",
...
def _(mo, sweep_combos, sweep_file, sweep_names, sweep_parallel):
    if len(sweep_combos) == 0:
        ...
        sweep_run_button = mo.ui.run_button(label=f"Run sweep ({len(sweep_combos)} combinations)")
        mo.output.replace(mo.vstack([
            mo.md(f"**{len(sweep_combos)} combinations** across {len(sweep_names)} parameters, {sweep_parallel.value} parallel jobs on `{sweep_file.value}`"),
            sweep_run_button,
        ]))
...
def _(
    ...
    if sweep_run_button.value and len(sweep_combos) > 0:
        ...
        _results = []
        _errors = []
        with mo.status.progress_bar(total=len(sweep_combos), title="Running sweep") as _bar:
            with ThreadPoolExecutor(max_workers=sweep_parallel.value) as _pool:
                ...
                for _future in as_completed(_futures):
                    ...
                    if _score is not None:
                        _row = {sweep_names[_i]: _combo[_i] for _i in range(len(sweep_names))}
                        _row["score"] = _score
        ...
        _parts = [mo.md(f"**Sweep complete:** {len(_results)} succeeded, {len(_errors)} failed")]
        if _errors:
            _err_lines = [f"- `{_c}`: {_e[:100]}" for _c, _e in _errors[:5]]
...
def _(csv_input, mo, pl):
    ...
    mo.stop(not csv_input.value.strip(), mo.md("*Run a sweep or paste CSV data to begin.*"))
...
def _(df, mo, param_cols, score_col):
    ...
    best_params = ", ".join(
        f"`{c}={best_row[c].item()}`" for c in param_cols
    )
    worst_params = ", ".join(
        f"`{c}={worst_row[c].item()}`" for c in param_cols
    )

    mo.vstack([
        mo.md(stats_md),
        mo.md(f"**Best combo:** {best_params} → **{best_row[score_col].item()}**"),
        mo.md(f"**Worst combo:** {worst_params} → **{worst_row[score_col].item()}**"),
    ])
    return
...
def _(alt, df, mo, param_cols, pl, score_col):
    ...
    for _param in param_cols:
        _param_agg = df.group_by(_param).agg([
...
def _(alt, df, mo, param_cols, pl, score_col):
    ...
    for _p in param_cols:
        _marginal[_p] = dict(
            df.group_by(_p).agg(pl.col(score_col).mean().alias("m"))
            .iter_rows()
        )
    ...
    for _i, _p1 in enumerate(param_cols):
        for _p2 in param_cols[_i + 1:]:
            ...
            _heat_df = _heat_df.with_columns(
                (
                    pl.col("mean_score")
                    - pl.col(_p1).map_elements(lambda v, _m=_marginal[_p1]: _m[v], return_dtype=pl.Float64)
                    - pl.col(_p2).map_elements(lambda v, _m=_marginal[_p2]: _m[v], return_dtype=pl.Float64)
                    + _grand_mean
                ).alias("interaction")
            )

            _max_abs = max(abs(_heat_df["interaction"].min()), abs(_heat_df["interaction"].max()), 0.1)

            _hm = alt.Chart(_heat_df).mark_rect().encode(
                x=alt.X(f"{_p1}:O", title=_p1),
                y=alt.Y(f"{_p2}:O", title=_p2),
                color=alt.Color(
                    "interaction:Q",
                    title="Interaction",
                    scale=alt.Scale(scheme="redblue", domain=[-_max_abs, _max_abs], reverse=True),
                ),
                tooltip=[
                    alt.Tooltip(_p1, title=_p1),
                    alt.Tooltip(_p2, title=_p2),
                    alt.Tooltip("interaction:Q", title="Interaction", format="+.1f"),
            ...
            _text = alt.Chart(_heat_df).mark_text(fontSize=11).encode(
                x=alt.X(f"{_p1}:O"),
                y=alt.Y(f"{_p2}:O"),
...
def _(df, mo, param_cols, pl, product, score_col):
    ...
    _sweep_df = pl.DataFrame(
        [dict(zip(param_cols, _combo)) for _combo in _new_combos]
    ) if _new_combos else pl.DataFrame()
    ...
    for _param in param_cols:
        ...
        _means = df.group_by(_param).agg(
            pl.col(score_col).mean().alias("mean")
        ).sort("mean", descending=True)
        _best_param_val = _means[_param][0]
        _best_mean = _means["mean"][0]
        _parts.append(
            mo.md(f"- **{_param}**: best mean score at `{_best_param_val}` ({_best_mean:.1f}), suggest exploring: `{_suggestions[_param]}`")
        )

    _parts.append(mo.md(f"### Suggested Next Sweep"))
    _parts.append(
        mo.md(f"**{len(_new_combos)} untested combinations** from refining around top performers:")
    )
